...
* 일일회의 슬랙 챗봇 HappyOntherBot봇이 주도한다. 슬랙 좌측하단 Apps에 있다. 추가적으로 트렐로를 이용하여 이슈를 추적한다.
* 스케줄은 오후 5:30 ~ 6:30 (답장하지 않을 시 15분마다 메시지를 받는다)
* 질문항목: 오늘 한 일, 내일 할 일, 어려운 점, 어젠더, 고마웠던 사람, 고마웠던 이유
...
* 질문메시지가 오면 질문에 대한 답을 한다.(챗봇에게 메시지를 보낸다.)
* 답을 한 메시지는 쓰레드 edit으로 수정가능하다.
* 대답을 모두 완료하면 슬랙 #회의채널에 쓰레드로 남겨진다.
* 45분 안에 답장하지 못하거나 챗봇에 문제가 있을 경우, 슬랙 #회의채널 쓰레드에 댓글로 내용을 직접 남긴다.
...
* 주간회의 슬랙 챗봇 HappyOntherBot봇이 주도한다. 슬랙 좌측하단 Apps에 있다. 추가적으로 트렐로를 이용하여 이슈를 추적한다.
* 스케줄은 KST 16:00 ~ 18:00 (답장하지 않을 시 15분마다 메시지를 받는다)
* 질문항목: 하지 말아야 될 일, 해야될 일, 차주에 할일, 차주 미팅or세미나or데모
  1. 목표에 관한 질문입니다   
...
* 월간 계획을 바탕으로 주간 목표의 우선순위를 수정할 수 있다.
